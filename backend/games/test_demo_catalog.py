...
class DemoCatalogTests(TestCase):
    ...
    def test_seed_is_idempotent_in_rows_and_files(self):
        self.seed()
        files = sorted(str(path.relative_to(self.media.name)) for path in Path(self.media.name).rglob("*.webp"))
        self.seed()
        self.assertEqual(Game.objects.count(), 12)
        self.assertEqual(GameScreenshot.objects.count(), 36)
        self.assertEqual(files, sorted(str(path.relative_to(self.media.name)) for path in Path(self.media.name).rglob("*.webp")))

    def test_seed_does_not_overwrite_existing_game_with_same_title(self):
        game = Game.objects.create(title="Aether Drift", developer="My real studio", price="2.34", description="Keep this text", release_date="2024-01-01")
        self.seed()
    ...
    def test_optional_demo_user_has_disjoint_collections_and_repeat_does_not_reset_password(self):
        ...
        self.assertFalse(owned & set(GameWishlist.objects.filter(user=user).values_list("game_id", flat=True)))
        self.assertFalse(owned & set(CartItem.objects.filter(cart__user=user).values_list("game_id", flat=True)))
    ...
    def test_existing_username_and_other_users_are_never_populated(self):
        User = get_user_model()
        existing = User.objects.create_user(username="steamnt_demo", email="existing@example.invalid", password="existing-only")
        other = User.objects.create_user(username="untouched", email="untouched@example.invalid", password="existing-only")
        self.seed(with_demo_user=True)
...
class GameGalleryTests(APITestCase):
    def setUp(self):
        self.game = Game.objects.create(title="Gallery test", developer="Demo", description="Example", price="1.00", release_date="2026-01-01")
    ...
    def test_gallery_is_ordered_and_urls_are_absolute(self):
        GameScreenshot.objects.create(game=self.game, position=2, image="games/b.webp", caption="Second")
        GameScreenshot.objects.create(game=self.game, position=0, image="games/a.webp", caption="First")
        response = self.client.get(self.url)
        self.assertEqual([image["caption"] for image in response.data["screenshots"]], ["First", "Second"])
        self.assertEqual(response.data["screenshots"][0]["image"], "http://testserver/media/games/a.webp")

    def test_gallery_does_not_leak_other_game_art(self):
        other = Game.objects.create(title="Other", developer="Demo", description="Other", price="1.00", release_date="2026-01-01")
    ...
    def test_detail_stays_read_only(self):
        user = get_user_model().objects.create_user(username="reader", email="reader@example.invalid", password="unused")
        self.client.force_authenticate(user=user)
        self.assertEqual(self.client.post(self.url, {"screenshots": []}, format="json").status_code, 405)
        self.assertEqual(self.client.patch(self.url, {"title": "Changed"}, format="json").status_code, 405)
