...
function CollectionDialog({
  ...
  const { dialogRef, handleKeyDown } = useModalFocus(onClose, submitting);
...
function LibraryPage() {
  ...
  const displayedItems = useMemo(() => {
    ...
    next = [...next].sort((first, second) => {
      if (sort === "title") {
        return (first.game?.title || "").localeCompare(
          second.game?.title || "",
        );
      }
      if (sort === "oldest") {
  ...
  return (
    <LibraryFrame
      ...
      pageHeader={
        <header className="store-page-title library-page-header">
          <h1>Library</h1>
        </header>
      }
      toolbar={
        <StoreToolbar
          ...
          sortOptions={[
            ...
            { value: "size", label: "Largest install" },
          ]}
          countLabel={`${displayedItems.length} ${displayedItems.length === 1 ? "game" : "games"}`}
          view={view}
          onView={setView}
        ...
        !loading && !error && items.length > 0 ? (
          <div className="library-collection-heading">
            ...
            >
              <button
                type="button"
                className={`ui-nav-tab${section === "all" ? " active" : ""}`}
              ...
              <button
                type="button"
                className={`ui-nav-tab${section === "favorites" ? " active" : ""}`}
              ...
              {collections.map((collection) => (
                <button
                  type="button"
                  key={collection.id}
                  className={`ui-nav-tab${section === `collection:${collection.id}` ? " active" : ""}`}
                  aria-pressed={section === `collection:${collection.id}`}
            ...
            {selectedCollection && (
              <button
                type="button"
                className="library-edit-collection"
    ...
    >
      ...
      {!loading && !error && items.length > 0 && (
        <>
          ...
          {(data.news.length > 0 || data.community.length > 0) && (
            <section className="library-updates" aria-label="Library updates">
              <EditorialSection
                title="News"
                linkLabel="All news"
                posts={data.news}
