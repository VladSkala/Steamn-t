...
export default function CommunityPostPage() {
  ...
  const [result, setResult] = useState({ key: "", post: null, error: "" });
    ...
    getCommunityPost(postId, { signal: controller.signal })
      .then((item) => {
        if (!controller.signal.aborted) {
          setResult({ key: requestKey, post: item, error: "" });
        }
      })
      .catch((requestError) => {
        ...
        setResult({ key: requestKey, post: null, error: message });
      });
  ...
  useEffect(() => {
    if (!loading && post && location.hash === "#comments") {
      document.getElementById("comments")?.focus({ preventScroll: false });
    }
      ...
      <section className="community-feed" aria-live="polite">
        ...
        {loading && (
          <CatalogFeedback
            ...
            message="Fetching the selected community post."
          />
        )}
        {!loading && error && (
          <CatalogFeedback
            ...
            message={
              error === "not-found"
                ? "This publication was removed, hidden, or does not exist."
                : error
            }
            onRetry={
              error === "not-found"
        ...
        {!loading && !error && post && (
          ...
          >
            <PostComments
              ...
              onRemoved={() => setResult(current => ({...current, post:{...current.post, comment_count: Math.max(0, current.post.comment_count - 1)}}))}
              onCreated={() =>
                setResult((current) => ({
                  ...current,
                  post: {
